...
┌───────────────────────────────────────┐
│ 1) ~/.open-agent/agent, ~/.claude, ...│
│ 2) <cwd>/.open-agent, <cwd>/.claude   │
└───────────────────────────────────────┘
                    │
                    ▼
        capability providers enumerate items
 (native provider scans project .open-agent before user .open-agent;
...
`CONFIG_DIR_NAME` is `.open-agent` (`packages/utils/src/dirs.ts`).
...
Walks parent directories upward and returns the **nearest existing directory per source base** (`.open-agent`, `.claude`, `.codex`, `.gemini`), then sorts results by source priority.
...
1. Global settings: `~/.open-agent/agent/config.yml`
...
1. Migrate from `~/.open-agent/agent/settings.json` (renamed to `.bak` on success)
...
- Native open-agent (`builtin.ts`): `100`
- Claude: `80`
...
native (.open-agent)   priority 100
claude                 priority  80
...
## 6) Native `.open-agent` provider behavior (`packages/coding-agent/src/discovery/builtin.ts`)

Native provider (`id: native`) reads native config from:

- project: `<cwd>/.open-agent/...`
- user: `~/.open-agent/agent/...`
...
- Skills scan `<ancestor>/.open-agent/skills` for each ancestor from the current working directory up to the repo root/home boundary, plus `~/.open-agent/agent/skills`, without requiring the root `.open-agent` directory itself to be non-empty.
- `SYSTEM.md` and `AGENTS.md` read user-level files directly and use nearest-ancestor project `.open-agent` lookup for project files, but the project `.open-agent` directory must be non-empty. See [`docs/system-prompt-customization.md`](./system-prompt-customization.md) for the full `SYSTEM.md` / `APPEND_SYSTEM.md` contract (replace vs. append, templating).

### Scope-specific loading

- Skills: `<ancestor>/.open-agent/skills/*/SKILL.md` and `~/.open-agent/agent/skills/*/SKILL.md`
...
## For `SYSTEM.md` and `AGENTS.md`, native provider uses nearest-ancestor project `.open-agent` directory search (walk-up) and still requires the project `.open-agent` dir to be non-empty.
