...
1. Capability providers (`toolCapability`), including:
   - Native open-agent config (`~/.open-agent/agent/tools`, `.open-agent/tools`)
   - Claude config (`~/.claude/tools`, `.claude/tools`)
   - Codex config (`~/.codex/tools`, `.codex/tools`)
   - Claude marketplace plugin cache provider
2. Installed plugin manifests (`~/.open-agent/plugins/node_modules/*` via plugin loader)
...
```ts
import type { CustomToolFactory } from "@open-agents/coding-agent";
...
- `pi`: injected `@open-agents/coding-agent` exports
