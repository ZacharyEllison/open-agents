...
```ts
import type { HookAPI } from "@open-agents/coding-agent/extensibility/hooks";
...
Discovery providers for `hookCapability` still model pre/post shell-style hook files (for example `.claude/hooks/pre/*`, `.open-agent/.../hooks/pre/*`).
...
`src/extensibility/hooks/index.ts` and the package subpath `@open-agents/coding-agent/extensibility/hooks` export:
...
The package root (`@open-agents/coding-agent`) does not re-export `HookAPI`; import legacy hook types from the hooks subpath.
