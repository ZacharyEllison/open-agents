# LSP configuration in open-agent

This guide explains how to configure language servers for the open-agent coding agent.
...
When no LSP config file is present, open-agent auto-detects servers by intersecting two conditions:
...
open-agent merges LSP config from multiple files, lowest to highest priority:

| Priority    | Location                                                                                                                    |
| ----------- | --------------------------------------------------------------------------------------------------------------------------- |
| 5 (lowest)  | `~/lsp.json`, `~/.lsp.json`, `~/lsp.yaml`, `~/.lsp.yaml`, `~/lsp.yml`, `~/.lsp.yml`                                         |
| 4           | Plugin LSP configs (marketplace / `--plugin-dir` roots)                                                                     |
| 3           | User config dirs: `~/.open-agent/agent/lsp.*`, `~/.claude/lsp.*`, `~/.codex/lsp.*`, `~/.gemini/lsp.*`                              |
| 2           | Project config dirs: `<project>/.open-agent/lsp.*`, `<project>/.claude/lsp.*`, `<project>/.codex/lsp.*`, `<project>/.gemini/lsp.*` |
...
- User-wide preferences → `~/.open-agent/agent/lsp.json`
- Project-specific overrides → `<project>/.open-agent/lsp.json`

> **Note:** Auto-detection is skipped only when at least one config file contributes server overrides. A config file that only sets `idleTimeoutMs` still lets open-agent auto-detect built-in servers. When server overrides exist, open-agent merges them with defaults and then loads servers that have matching `rootMarkers`, an available binary, and are not explicitly `disabled`.
...
The `capabilities` object enables optional server-specific features that open-agent supports on a per-server basis:
...
Place the override in `<project>/.open-agent/lsp.json`:
...
The user-level config in `~/.open-agent/agent/lsp.json` is unaffected; pylsp is only suppressed in this project.
