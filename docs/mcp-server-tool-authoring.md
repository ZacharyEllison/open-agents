...
```text
Config sources (.open-agent/.claude/.cursor/.vscode/mcp.json, mcp.json, etc.)
...
In practice MCP servers also come from higher-priority providers (for example native `.open-agent/...` and tool-specific config dirs). Authoring guidance:

- Prefer `.open-agent/mcp.json` (project) or `~/.open-agent/agent/mcp.json` (user) for explicit control.
...
open-agent-native MCP config (`.open-agent/mcp.json`, `~/.open-agent/agent/mcp.json`, plus their `.mcp.json` variants) expands `${VAR}` and `${VAR:-default}` placeholders recursively before converting to runtime config. It also accepts boolean/string forms for `enabled` (`true`, `false`, `1`, `0`) and numeric strings for `timeout`.
