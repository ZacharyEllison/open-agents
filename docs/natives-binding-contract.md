...
This document defines the JS/TS contract between `@open-agents/natives` callers and the loaded N-API addon.
...
Consumers in `packages/coding-agent` and `packages/tui` import directly from `@open-agents/natives`.
