...
- Examples (adjust to match the actual packages you are porting):
  - `@mariozechner/pi-coding-agent` → `@open-agents/coding-agent`
  - `@mariozechner/pi-agent-core` → `@open-agents/agent`
  - `@mariozechner/pi-tui` → `@open-agents/tui`
  - `@mariozechner/pi-ai` → `@open-agents/ai`
...
- In `packages/coding-agent`, use `logger` from `@open-agents/utils` for internal/runtime logging; CLI command files may use `console.*` for intentional user-facing output.
...
- Prefer existing helpers and utilities over new ad-hoc code.
  ...
  - TypeScript packages generally use source files directly; `@open-agents/natives` exports generated native bindings from `packages/natives/native`.
...
| Upstream                                           | Our Fork                                                  | Reason                                        |
| -------------------------------------------------- | --------------------------------------------------------- | --------------------------------------------- |
| `clipboard.ts` + `clipboard-image.ts` (tool files) | `src/utils/clipboard.ts` backed by `@open-agents/natives` | Native implementation with a small TS wrapper |
...
- `clipboard-image.ts` — image clipboard support is exposed through `src/utils/clipboard.ts` backed by `@open-agents/natives`
