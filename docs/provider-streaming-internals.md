...
This document explains how token/tool streaming is normalized in `@open-agents/ai`, then propagated through `@open-agents/agent` and `coding-agent` session events.
...
## Unified stream contract in `@open-agents/ai`
