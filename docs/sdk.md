...
The SDK is the in-process integration surface for `@open-agents/coding-agent`.
...
```bash
bun add @open-agents/coding-agent
```

## Entry points

`@open-agents/coding-agent` exports the SDK APIs from the package root (and also via `@open-agents/coding-agent/sdk`).
...
```ts
import { createAgentSession } from "@open-agents/coding-agent";
...
- `cwd`: `getProjectDir()`
- `agentDir`: `~/.open-agent/agent` (via `getAgentDir()`)
...
```ts
import { createAgentSession, SessionManager } from "@open-agents/coding-agent";
...
```ts
import { SessionManager } from "@open-agents/coding-agent";
