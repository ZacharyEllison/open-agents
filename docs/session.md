...
```text
~/.open-agent/agent/sessions/--<cwd-encoded>--/<timestamp>_<sessionId>.jsonl
```
...
```text
~/.open-agent/agent/blobs/<sha256>
```

Terminal breadcrumb files are written under:

```text
~/.open-agent/agent/terminal-sessions/<terminal-id>
```
...
- `listAll()` -> sessions across all project scopes under `~/.open-agent/agent/sessions`
...
- DB: `~/.open-agent/agent/history.db`
