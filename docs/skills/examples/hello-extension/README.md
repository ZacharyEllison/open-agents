...
```
cp -r . ~/.open-agent/agent/extensions/hello-extension
```

Restart `open-agent`. You will see the startup notification immediately.
...
```
open-agent --extension ./hello-extension
```

## Usage

After loading, type `/hello` or `/hello Ada` in the open-agent prompt. The command sends a visible greeting custom message into the conversation and shows a "Message sent!" notification.
