...
An `open-agent` extension that demonstrates `tool_call` blocking. It intercepts `bash` tool calls and returns `{ block: true, reason: "..." }` when the command contains `rm -rf /` with normal whitespace, preventing the tool from executing.
...
```
cp -r . ~/.open-agent/agent/extensions/safety-hook
```

Restart `open-agent`. The hook is active for all sessions.

Or load once:

```
open-agent --extension ./safety-hook
```
