...
`discovery.ts` header comments still mention `.pi` and do not mention `.codex`/`.gemini`. Actual runtime order is driven by `src/config.ts` and currently uses `.open-agent`, `.claude`, `.codex`, `.gemini`.
...
- Higher-priority source family overrides lower (`.open-agent` before `.claude`, etc.).
...
`OA_BLOCKED_AGENT` is read at tool construction. If request matches, execution is rejected with recursion-prevention message.
