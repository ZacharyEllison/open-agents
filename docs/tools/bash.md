...
| `pty` | `boolean` | No | Request PTY mode. Default `false`. PTY is used only when `pty: true`, `OA_NO_PTY !== "1"`, and the tool context has a UI. |
...
3. Foreground PTY
   - Requires `pty: true`, UI context, and `OA_NO_PTY !== "1"`.
...
- Subprocesses / native bindings / client terminal
  - Non-PTY local execution uses native shell execution via `@open-agents/natives` (`Shell.run()` or `executeShell()`).
...
- PTY mode is ignored in non-UI contexts and when `OA_NO_PTY=1`; the tool silently falls back to non-PTY execution.
