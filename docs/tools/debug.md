...
- Subprocesses / native bindings
  ...
  - `getWorkProfile(30)` comes from `@open-agents/natives`.
  - CPU profiling uses `node:inspector/promises`; heap snapshots use `Bun.generateHeapSnapshot("v8")`; raw/log viewers sanitize text via `@open-agents/natives`.
