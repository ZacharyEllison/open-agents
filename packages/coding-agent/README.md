# @open-agents/coding-agent

Core implementation package for the `open-agent` coding agent in the `open-agents` monorepo.
...
The agent supports three mutually-exclusive memory backends, selected via the `memory.backend` setting (Settings → Memory tab, or `~/.open-agent/agent/config.yml`):
