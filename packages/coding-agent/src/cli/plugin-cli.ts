...
import { APP_NAME, getProjectDir } from "@open-agents/utils";
...
async function handleInstall(
	...
	if (packages.length === 0) {
		...
		console.error(chalk.dim(`  ${APP_NAME} plugin install @open-agents/swarm-extension`));
...
${chalk.bold("Examples:")}
  ${APP_NAME} plugin install @open-agents/swarm-extension
