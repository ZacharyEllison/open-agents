...
export function normalizeTinyModelDevice(value: string | undefined): TinyModelDevice | undefined {
	...
	throw new Error(
		`Unsupported OA_TINY_DEVICE=${JSON.stringify(value)}. Use cpu, gpu, metal, webgpu, auto, cuda, dml, coreml, wasm, webnn, webnn-gpu, webnn-cpu, or webnn-npu.`,
	);
}

export function resolveTinyModelDevicePreference(
	value: string | undefined = $env.OA_TINY_DEVICE,
...
/**
 * Map a `providers.tinyModelDevice` setting value onto a `OA_TINY_DEVICE` env
