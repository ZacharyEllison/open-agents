...
/**
 * Validate and canonicalize a `OA_TINY_DTYPE` value. Returns `undefined` when
...
export function normalizeTinyModelDtype(value: string | undefined): TinyModelDtype | undefined {
	...
	throw new Error(
		`Unsupported OA_TINY_DTYPE=${JSON.stringify(value)}. Use auto, fp32, fp16, q8, int8, uint8, q4, bnb4, q4f16, q2, q2f16, q1, or q1f16.`,
	);
}

/**
 * Resolve the `OA_TINY_DTYPE` override. `undefined` means "use the per-model spec
...
export function resolveTinyModelDtypeOverride(
	value: string | undefined = $env.OA_TINY_DTYPE,
...
/**
 * Map a `providers.tinyModelDtype` setting value onto a `OA_TINY_DTYPE` env value
