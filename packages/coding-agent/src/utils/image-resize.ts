...
/**
 * Read `OA_NO_WEBP` per-call so runtime toggles take effect.
 ...
 *
 * Set OA_NO_WEBP to exclude WebP from encoding (llama.cpp STB doesn't decode it).
 *
...
export async function resizeImage(img: ImageContent, options?: ImageResizeOptions): Promise<ResizedImage> {
	...
	try {
		...
		// WebP usually beats JPEG by 25–35% but is disabled when OA_NO_WEBP is set
		...
		// unless OA_NO_WEBP is set (llama.cpp STB incompatibility).
