...
const ORIGINAL_TIMEOUT = process.env.OA_MCP_TIMEOUT_MS;

afterEach(() => {
	if (ORIGINAL_TIMEOUT === undefined) {
		delete process.env.OA_MCP_TIMEOUT_MS;
	} else {
		process.env.OA_MCP_TIMEOUT_MS = ORIGINAL_TIMEOUT;
	}
});
...
describe("MCP timeout configuration", () => {
	...
	test("rejects non-numeric env values and falls back to the default", () => {
		process.env.OA_MCP_TIMEOUT_MS = "not-a-number";
		const warn = spyOn(logger, "warn").mockImplementation(() => {});
