...
import { getBundledModel, type Model } from "@open-agents/ai";
import type { ModelRegistry } from "@open-agents/coding-agent/config/model-registry";
import { Settings } from "@open-agents/coding-agent/config/settings";
import { ModelSelectorComponent } from "@open-agents/coding-agent/modes/components/model-selector";
import { getThemeByName, setThemeInstance } from "@open-agents/coding-agent/modes/theme/theme";
...
describe("ModelSelector role badge thinking display", () => {
	...
	test("shows tier badges and honors modelTags metadata overrides", async () => {
		installTestTheme();
		...
		const settings = Settings.isolated({
			modelTiers: {
				interface: `${model.provider}/${model.id}`,
				worker: `${model.provider}/${model.id}:low`,
				compactor: `${model.provider}/${model.id}`,
			},
			modelTags: {
				compactor: { name: "Quick", color: "error" },
			},
		});
