...
describe("tinyWorkerEnvOverlay", () => {
	...
	it("lets a present env var win over the persisted setting", () => {
		expect(tinyWorkerEnvOverlay({ OA_TINY_DEVICE: "cpu" }, "cuda", "fp16")).toEqual({ OA_TINY_DTYPE: "fp16" });
		expect(tinyWorkerEnvOverlay({ OA_TINY_DTYPE: "q8" }, "cuda", "fp16")).toEqual({ OA_TINY_DEVICE: "cuda" });
	});
