...
import { canUseInteractiveBashPty } from "@open-agents/coding-agent/tools/bash-pty-selection";

const originalPlatform = process.platform;
const originalNoPty = Bun.env.OA_NO_PTY;
...
function setNoPty(value: string | undefined): void {
	if (value === undefined) {
		delete Bun.env.OA_NO_PTY;
		return;
	}
	Bun.env.OA_NO_PTY = value;
}
