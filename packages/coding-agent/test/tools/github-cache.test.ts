 ...
 *
 * Each test isolates `OA_GITHUB_CACHE_DB` to a temp file and clears
...
import { getOrFetchIssue, getOrFetchPr } from "@open-agents/coding-agent/tools/gh";
import {
	clearAll,
	getCached,
	getOrFetchView,
...
} from "@open-agents/coding-agent/tools/github-cache";
...
beforeEach(async () => {
	...
	originalEnv = process.env.OA_GITHUB_CACHE_DB;
	process.env.OA_GITHUB_CACHE_DB = path.join(tempDir, "github-cache.db");
	resetCacheForTests();
});
...
afterEach(async () => {
	...
	if (originalEnv === undefined) {
		delete process.env.OA_GITHUB_CACHE_DB;
	} else {
		process.env.OA_GITHUB_CACHE_DB = originalEnv;
	}
	vi.restoreAllMocks();
...
describe("github-cache db layer", () => {
	...
	it("does not chmod an existing cache parent directory", async () => {
		...
		process.env.OA_GITHUB_CACHE_DB = path.join(parent, "github-cache.db");
		resetCacheForTests();
