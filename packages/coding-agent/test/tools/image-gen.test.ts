...
import type { CustomToolContext } from "@open-agents/coding-agent/extensibility/custom-tools";
import type { ReadonlySessionManager } from "@open-agents/coding-agent/session/session-manager";
import { imageGenTool, setPreferredImageProvider } from "@open-agents/coding-agent/tools/image-gen";
...
describe("imageGenTool", () => {
	...
	it("routes xAI image generation with xAI-only aspect ratios", async () => {
		...
		expect(captured.userAgent).toBe("open-agent/xai");
		expect(requestBody).toMatchObject({
