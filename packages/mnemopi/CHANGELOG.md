...
- Published `@open-agents/mnemopi` to npm: the local SQLite memory engine is now built, checked, tested, and released through the monorepo CI pipeline alongside the other workspace packages.
- Exported the diagnostic inspector as the `@open-agents/mnemopi/diagnose` subpath for coding-agent memory maintenance commands.
