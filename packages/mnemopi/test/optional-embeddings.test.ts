	...
	it("calls an OpenAI-compatible custom embeddings endpoint without requiring an API key", async () => {
		...
		const server = Bun.serve({
			...
			fetch: async request => {
				...
				expect(request.headers.get("user-agent")).toBe(`open-agent/${packageJson.version}`);
				expect(request.headers.get("http-referer")).toBe("https://github.com/ZacharyEllison/open-agents/");
				expect(request.headers.get("x-openrouter-title")).toBe("open-agent");
