...
/**
 * Native addon loader for `@open-agents/natives`.
 *
...
function resolveLeafPackageDir(platformTag) {
	try {
		...
		return path.dirname(require_.resolve(`@open-agents/natives-${platformTag}/package.json`));
	} catch {
		return null;
	}
}
 ...
 * `node_modules/@open-agents/natives/native/`, leaving an old binary next to a
...
function validateLoadedBindings(ctx, bindings, candidate) {
	...
	throw new Error(
		`Loaded ${candidate} but it does not expose the @open-agents/natives@${ctx.packageVersion} ` +
...
function buildHelpMessage(ctx) {
	if (ctx.isCompiledBinary) {
		...
		const downloadHints = ctx.addonFilenames
			.map(filename => {
				const downloadUrl = `https://github.com/ZacharyEllison/open-agents/releases/latest/download/${filename}`;
				const targetPath = path.join(ctx.versionedDir, filename);
	...
	return (
		"If installed via npm/bun, try reinstalling: bun install @open-agents/natives\n" +
