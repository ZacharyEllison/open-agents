...
Multi-agent orchestration for open-agent. Define agent workflows in YAML — pipelines, parallel fan-outs, sequential chains, or any DAG — and run them unattended until completion.

Each agent is a full open-agent subagent with access to every tool: bash, python, read, write, edit, grep, find, fetch, web_search, browser. The orchestrator manages lifecycle and ordering; agents communicate through the shared workspace filesystem.
...
### Inside open-agent (TUI)

Register the extension in your config (`~/.open-agent/config.json` or `.open-agent/config.json`):
...
Each agent is a full open-agent session. It can:
...
src/swarm/
  ...
  executor.ts         Spawns agents via open-agent's runSubprocess
